...
for algo in algos:
    ...
    for numref, marker, color in zip(['1_REF', '4_REF', '8_REF'], ['-D', '-o', '-^'], ["#e42536", "#f89c20", "#5790fc",]):#["#5790fc", "#f89c20","#7a21dd"]):#['purple', 'yellow', 'orange']):# zip(['8_REF'], ['-^'], ['orange']):
        ...
        # for cut in cuts:
        #     print(np.count_nonzero(df_b >= cut, axis=1))
        #     for index, row in df_b.iterrows():
        #         iii = 0
        #         for h, hcut, hn in zip(row, cut, df_b.columns):
        #             if h >= hcut:
        #                 iii=iii+1
        #                 print(iii)
        #                 print(hn)
